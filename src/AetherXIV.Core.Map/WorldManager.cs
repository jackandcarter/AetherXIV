...
namespace AetherXIV.Core.Map
{
    class WorldManager
    {
                ...
                try
                {
                    ...
                    foreach (var zonePair in zoneList)
                    {
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                int actorId = checked((int)zone.AllocateSpawnedActorNumber());
        ...
        public bool DoSeamlessZoneChange(Player player, uint destinationZoneId)
        {
            ...
            if (ChocoboStopPolicy.ShouldRefuseTransition(player.GetMountState(), newZone.canRideChocobo))
            {
                DevDiagnostics.Trace(
                    "zone.seamless.change.blocked",
                    "reason", "mounted destination does not permit rides",
        ...
        public bool SeamlessCheck(Player player)
        {
            ...
            foreach (SeamlessBoundry bounds in seamlessBoundryList[regionId])
            {
                ...
                bool playerIsInZone1 = player.zoneId == bounds.zoneId1;
                float destinationX1 = playerIsInZone1 ? bounds.zone2_x1 : bounds.zone1_x1;
                float destinationZ1 = playerIsInZone1 ? bounds.zone2_y1 : bounds.zone1_y1;
                float destinationX2 = playerIsInZone1 ? bounds.zone2_x2 : bounds.zone1_x2;
                float destinationZ2 = playerIsInZone1 ? bounds.zone2_y2 : bounds.zone1_y2;
                float currentZoneX1 = playerIsInZone1 ? bounds.zone1_x1 : bounds.zone2_x1;
                float currentZoneZ1 = playerIsInZone1 ? bounds.zone1_y1 : bounds.zone2_y1;
                float currentZoneX2 = playerIsInZone1 ? bounds.zone1_x2 : bounds.zone2_x2;
                float currentZoneZ2 = playerIsInZone1 ? bounds.zone1_y2 : bounds.zone2_y2;
                uint destinationZoneId = playerIsInZone1 ? bounds.zoneId2 : bounds.zoneId1;

                if (SeamlessBoundaryPolicy.ContainsPoint(
                        player.positionX,
                        player.positionZ,
                        currentZoneX1,
                        currentZoneZ1,
                        ...
                        currentZoneZ2))
                {
                    return ClearMergedZone(player, playerIsInZone1 ? "entered-primary-zone-box" : "entered-secondary-zone-box");
                }

                if (SeamlessBoundaryPolicy.ReachedDestination(
                        player.zoneId2 == destinationZoneId,
                        player.oldPositionX,
                        player.oldPositionZ,
                        player.positionX,
                        player.positionZ,
                        ...
                        bounds.merge_y2))
                {
                    Zone destinationZone = GetZone(destinationZoneId);
                    if (destinationZone != null
                        && ChocoboStopPolicy.ShouldRefuseTransition(player.GetMountState(), destinationZone.canRideChocobo))
                    {
                        return RefuseMountedSeamlessTransition(player, destinationZoneId);
                    }

                    return DoSeamlessZoneChange(player, destinationZoneId);
                }

                bool movementTouchesMerge = SeamlessBoundaryPolicy.ContainsPoint(
                                                player.positionX,
                                                player.positionZ,
                                                bounds.merge_x1,
                                                bounds.merge_y1,
                                                bounds.merge_x2,
                                                bounds.merge_y2)
                                            || SeamlessBoundaryPolicy.MovementIntersectsBounds(
                                                player.oldPositionX,
                                                player.oldPositionZ,
                                                player.positionX,
                                                player.positionZ,
                ...
                if (movementTouchesMerge)
                {
                    return MergeZones(player, destinationZoneId);
                }

                if (player.zoneId2 == destinationZoneId
                    && SeamlessBoundaryPolicy.IsWithinTransitionCorridor(
                        player.positionX,
                        player.positionZ,
                        destinationX1,
                        destinationZ1,
        ...
        private bool RefuseMountedSeamlessTransition(Player player, uint destinationZoneId)
        {
            float rejectedX = player.positionX;
            float rejectedY = player.positionY;
            float rejectedZ = player.positionZ;
            float safeX = player.oldPositionX;
            float safeY = player.oldPositionY;
            float safeZ = player.oldPositionZ;
            float safeRotation = player.oldRotation;

            if (Single.IsNaN(safeX) || Single.IsInfinity(safeX)
                || Single.IsNaN(safeY) || Single.IsInfinity(safeY)
                || Single.IsNaN(safeZ) || Single.IsInfinity(safeZ))
            {
                return false;
            }

            player.oldPositionX = rejectedX;
            player.oldPositionY = rejectedY;
            player.oldPositionZ = rejectedZ;
            player.positionX = safeX;
            player.positionY = safeY;
            player.positionZ = safeZ;
            player.rotation = safeRotation;
            player.positionUpdates.Clear();

            if (player.zone != null)
                player.zone.UpdateActorPosition(player);
            if (player.zone2 != null && player.zone2 != player.zone)
                player.zone2.UpdateActorPosition(player);

            ClearMergedZone(player, "mounted-chocobo-refused-destination");
            player.playerSession.QueuePacket(MoveActorToPositionPacket.BuildPacket(
                player.actorId,
                safeX,
                safeY,
                safeZ,
        ...
        public void DoZoneChange(Player player, uint destinationZoneId, string destinationPrivateArea, int destinationPrivateAreaType, byte spawnType, float spawnX, float spawnY, float spawnZ, float spawnRotation)
        {
            ...
            // City/interior/dungeon transitions cannot carry a ride state.
            // The three stablemaster exits target ride-enabled field zones and
            // therefore preserve the mount established before transport.
            if (player.GetMountState() != 0 && !newArea.canRideChocobo)
                ChocoboService.EndRide(player);
        ...
        public void DoZoneChangeContent(Player player, PrivateAreaContent contentArea, float spawnX, float spawnY, float spawnZ, float spawnRotation, ushort spawnType = SetActorPositionPacket.SPAWNTYPE_WARP_DUTY)
        {
            ...
            // Instanced content never inherits an overworld chocobo ride. The
            // ordinary zone-change path performs the equivalent canRide check.
        ...
        public void DoZoneIn(Player player, bool isLogin, ushort spawnType)
        {
            DevDiagnostics.Trace(
                ...
                "z", player.positionZ,
                "rot", player.rotation);
            player.PrepareZoneChangePositionValidation(
                player.zoneId,
                player.positionX,
                player.positionY,
                player.positionZ,
            ...
            if (playerArea == null && requestedPrivateArea)
            {
                ...
                if (TryRestoreOpeningTutorialArea(player, out Area restoredArea, out string recoveryReason))
                {
                    DevDiagnostics.Trace(
                        "zone.in.restoredOpeningTutorialArea",
                        "player", player.customDisplayName,
                        "requestedZone", player.zoneId,
                        "requestedPrivateArea", missingPrivateArea ?? "",
                        "requestedPrivateAreaType", missingPrivateAreaType,
                        "restoredArea", restoredArea.zoneName,
                        "restoredAreaKind", restoredArea.GetType().Name,
                        "recoveryReason", recoveryReason);
                    playerArea = restoredArea;
                }
            }
            ...
            if (player.GetMountState() != 0 && !playerArea.canRideChocobo)
                ChocoboService.EndRide(player, false);
        ...
        private bool TryRestoreOpeningTutorialArea(Player player, out Area restoredArea, out string reason)
        {
            restoredArea = null;
            reason = "";
            ...
            string directorPath;
            uint postBattleZone;
            uint postBattleLevel;
            float postBattleX;
            float postBattleY;
            float postBattleZ;
            float postBattleRotation;

            switch (player.privateArea)
            {
                case "SimpleContent30002":
                    openingQuestId = 110001;
                    contentScript = "man0l01";
                    directorPath = "Quest/QuestDirectorMan0l001";
                    postBattleZone = 230;
                    postBattleLevel = 1;
                    postBattleX = -826.868469f;
                    postBattleY = 6f;
                    postBattleZ = 193.745865f;
                    postBattleRotation = -0.008368492f;
                    break;
                case "SimpleContent30010":
                    openingQuestId = 110005;
                    contentScript = "man0g01";
                    directorPath = "Quest/QuestDirectorMan0g001";
                    postBattleZone = 155;
                    postBattleLevel = 1;
                    postBattleX = 175.38f;
                    postBattleY = -1.21f;
                    postBattleZ = -1156.51f;
                    postBattleRotation = -2.1f;
                    break;
                case "SimpleContent30079":
                    openingQuestId = 110009;
                    contentScript = "man0u01";
                    directorPath = "Quest/QuestDirectorMan0u001";
                    postBattleZone = 175;
                    postBattleLevel = 3;
                    postBattleX = -22.81f;
                    postBattleY = 196f;
                    postBattleZ = 87.82f;
                    postBattleRotation = 2.98f;
                    break;
                default:
                    return false;
            }

            Quest openingQuest = player.GetQuest(openingQuestId);
            ...
            if (openingQuest.GetPhase() >= 10)
            {
                ...
                player.zoneId = postBattleZone;
                player.privateArea = "PrivateAreaMasterPast";
                player.privateAreaType = postBattleLevel;
                player.oldPositionX = player.positionX = postBattleX;
                player.oldPositionY = player.positionY = postBattleY;
                player.oldPositionZ = player.positionZ = postBattleZ;
                player.oldRotation = player.rotation = postBattleRotation;
                reason = "phase-10-post-battle";
                return true;
            }

            Zone parentZone = GetZone(player.zoneId);
            if (parentZone == null)
                return false;

            string privateAreaName = player.privateArea;
            PrivateAreaContent contentArea = parentZone.CreateContentArea(
                player,
        ...
        public bool StopZoneThread(int timeoutMilliseconds)
        {
            ...
            using (ManualResetEvent callbacksFinished = new ManualResetEvent(false))
            {
