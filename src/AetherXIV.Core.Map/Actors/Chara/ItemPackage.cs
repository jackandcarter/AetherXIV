...
namespace AetherXIV.Core.Map.actors.chara.player
{
    ...
    {       
        ...
        // Applies a database transaction that was already committed by a
        // server-owned gameplay service. These helpers deliberately perform no
        // database writes; they only bring the authoritative in-memory package
        // into line and mark the exact client slots dirty.
        public void ApplyCommittedAdd(InventoryItem item)
        {
            if (item == null || endOfListIndex >= list.Length)
                return;
            item.SetOwner(owner, itemPackageCode, (ushort)endOfListIndex);
            list[endOfListIndex] = item;
            isDirty[endOfListIndex] = true;
            endOfListIndex++;
        }

        public void ApplyCommittedRemove(InventoryItem item)
        {
            if (item == null || item.itemPackage != itemPackageCode || item.slot >= endOfListIndex)
                return;
            ushort removedSlot = item.slot;
            if (list[removedSlot] == null || list[removedSlot].uniqueId != item.uniqueId)
                return;
            list[removedSlot].ClearOwner();
            list[removedSlot] = null;
            isDirty[removedSlot] = true;
            DoRealign(false);
        }
