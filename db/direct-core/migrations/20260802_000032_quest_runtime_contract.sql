-- Restore the quest availability catalog used by the legacy quest-state manager.
-- The 2.0 completion ledger remains row-per-quest; only its logical uniqueness
-- is enforced so repeated completion cannot create duplicate state.
...
DELETE newer
FROM `characters_quest_completed` newer
JOIN `characters_quest_completed` older
  ON older.`characterId` = newer.`characterId`
 AND older.`questId` = newer.`questId`
 AND older.`id` < newer.`id`;

ALTER TABLE `characters_quest_completed`
  ADD UNIQUE KEY IF NOT EXISTS `uq_characters_quest_completed_character_quest`
    (`characterId`, `questId`);
