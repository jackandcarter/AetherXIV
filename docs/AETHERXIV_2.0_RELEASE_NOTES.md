...
- **Build 21999:** restores the complete Gridania opening sequence and brings the private-to-public Canopy boundary in line with the captured retail room-exit state machine. The opening exit presents the two instance-help messages, the private Canopy retains its optional adventurer conversations, and Miounne runs the full `processEvent110` / `man0g110` Bentbranch briefing before granting the linkpearl and advancing to phase 5. The room exit sends state `0x0F` without an actor wipe, holds destination actor delivery for the observed six-second teardown, sends the player/inventory bootstrap, then streams at most eight destination actors every 140 ms before committing the mass-delete keep list. Map relay records are grouped into compressed multi-subpacket World frames bounded by the retail `0xFE0` uncompressed-body limit, preventing ordinary new-character login from merging several client bootstrap frames into one oversized frame. The captured `0x000A` fixed 32-ID keep-list body is followed by counted `0x0008` bodies and `0x0007`; `0x000A` is no longer misidentified as a required primary actor table. Only the client's transition-ready `0x0007` acknowledgement with trailing value `-1` releases the destination director notice. `processEventTu_001` then uses the captured `0xB0` response-bearing `RunEventFunction` form and retains the director owner until the client returns `0x012E EventUpdate`; only then does the coroutine close the notice. Event traces now correlate the pending function, elapsed time, and whether a client response preceded `EndEvent`. Client-to-server `0x0130` is decoded separately as the `{actorId, 0x2711, 0, 0}` list-object lifecycle acknowledgement emitted by the actor-spawn pipeline; it requires no response and no longer appears as an unknown event-like opcode. The later linkpearl click starts its own `commandRequest` event and routes to `Man0g1.onNpcLS`; the phase-5 character-specific login repair remains removed so new characters exercise the original phase-zero flow.
- **Build 21998:** attempted to restore Miounne's opening presentation but did not combine the full briefing with the subsequent linkpearl grant and tutorial transition; Build 21999 supersedes that incomplete sequence.
- **Build 21991:** fixes the Gridania opening-battle handoff so the deferred tutorial event waits for the client's completed zone-in acknowledgement instead of racing an earlier position update. Interrupted characters in the three-wolf duty now recover beside Yda at the valid battle entry point rather than remaining stranded in a stale private copy.
...
- 569 automated Core, protocol, data, launcher, server-management and Umbra checks passing for this release snapshot.
