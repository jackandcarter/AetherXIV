...
function onEventStarted(player, npc, triggerName)
    ...
    playerCurrentRank = player:GetCurrentGrandCompanyRank();
            ...
            if (eventTalkChoice == 1) then 
                ...
                while (true) do
                    ...
                    if (buyIndex == -1) then
                        ...
                        if (entry == nil or entry[6] ~= playerGC or entry[5] > playerCurrentRank or (playerCurrentRank == 127 and entry[5] > 0)) then
                            callClientFunction(player, "eventShopMenuClose");
                            break;
                        elseif (entry[8] == 4) then
                            location = INVENTORY_KEYITEMS;
                        else    
                            location = INVENTORY_NORMAL;  
                        end
                        if (entry[1] == 2001004 or entry[1] == 2001005 or entry[1] == 2001006) then
                            -- The issuance is a progression-critical unique key item. Keep the
                            -- original client shop flow, but commit the seal debit and key-item
                            -- grant atomically on the server after revalidating company, rank,
                            -- selection, inventory space, and current seal balance.
                            local purchaseResult = player:TryPurchaseChocoboIssuance(npcId, entry[1], entry[4]);
                            if (purchaseResult == 0) then
                                player:SendGameMessage(player, GetWorldMaster(), 25275, MESSAGE_TYPE_SYSTEM, entry[1], entry[2], entry[3], entry[4], player.gcCurrent);
                            elseif (purchaseResult == 5) then
                                player:SendGameMessage(player, GetWorldMaster(), 25065, MESSAGE_TYPE_SYSTEM);
                            elseif (purchaseResult == 6) then
                                player:SendGameMessage(player, GetWorldMaster(), 40279, MESSAGE_TYPE_SYSTEM, entry[1], entry[2]);
                            elseif (purchaseResult == 7) then
                                player:SendGameMessage(player, GetWorldMaster(), 60022, MESSAGE_TYPE_SYSTEM);
                            else
                                callClientFunction(player, "eventShopMenuClose");
                                break;
                            end
